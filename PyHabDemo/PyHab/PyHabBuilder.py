...
class PyHabBuilder:
    ...
    TODO: Make conditions text to make condition recording more interpreteble. Other words, do movie names instead of numbers.
    """
    ...
    def condMaker(self, rep=False):
        ...
        for i in range(0,len(self.trialTypesArray['labels'])):
            ...
            # TODO: Change movtext to be less...awful. Or just get rid of it!
        ...
        for j in range(0, len(self.settings['condList'])): #condition labels. Here's where rubber meets road!
            ...
            for q in range(0,len(self.trialTypesArray['labels'])):
                if self.trialTypesArray['labels'][q] in condContent[j].keys():
                    ...
                    tempTxt = []
                tempText = visual.TextStim(self.win, text=tempTxt, height=intervalVert*.35,pos=[tTypeHeaders[q].pos[0],vpos], alignHoriz='center')
                drawConds.append(tempText)
    ...
    def condSetter(self, cond='NEW', ex=False):
        ...
        The output of this is a dict with an array like this for each trial type: [1, 2, 3]
                ...
                if condTyDlg.OK:
                    ...
                    for q in range(1, maxNum+1):
                        if condTyInfo.count(q) > 1:
                            ...
                            i -= 1 # This is why our for loop became a while.
                            irrel = errDlg.show()
                            stop = True
                    if maxNum == 0:
                        errDlg = gui.Dlg(title="Warning, invalid order!")
                    ...
                    if not stop:
                        ...
                        for q in range(1, maxNum+1):
                            try:
                                tMov = condTyInfo.index(q) # Adds the movie indexes (+1) to the order
                                tempOrder.append(tMov+1) # TODO: Change to the movie name from stimnames.
                            except ValueError:
                                errDlg = gui.Dlg(title="Warning, invalid order!")
                                ...
                                stop = True
                                break
                        if not stop: # Stops it from saving bad orders due to non-consecutive numbering.
